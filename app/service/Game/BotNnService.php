...
class BotNnService extends BaseGameService
{
    public function Betting(string $command, string $crowd, array $tgUser)
    {
        ...
        foreach ($array as $key => $value) {
            ...
            if (!isset(GameModel::NN_ODDS[$value[0]])) {
                //判断 赔率ID 是否存在，不存在直接 退出
                return false;
            }
            $res['rate_id'] = GameModel::NN_ODDS[$value[0]];//查询游戏配率ID
        ...
        $tableId = GameModel::getInstance()->getTableId($crowd, 'nn');
        if (empty($tableId)) {
        ...
        $post['tg_id'] = $tgID;
        //2 获取下注游戏类型 //固定下注类型
        $post['game_type'] = GameModel::NN_ODDS;

        $request = betPostData($post);
        $res = GameFacade::bettingPostNN($request, '/tg/user_bet_order');
        //判断返回数据是否正常   //处理接口返回数据
        if (empty($res)) {
            traceLog($res, '下注请求失败--');
            //tguser -- 下注失败--原因
